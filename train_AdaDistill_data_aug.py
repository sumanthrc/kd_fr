...
from backbones import net # to load adaface res50 teacher

#imports realted to AroFace augmentation
from utils.Aroface_data_aug.partial_fc_v2 import PartialFC_V2
from utils.Aroface_data_aug.AdvWarp import adversarial_img_warping
from utils.Aroface_data_aug.aroface_losses import CombinedMarginLoss
...
def main(args):
    ...
    if getattr(cfg, "use_adaface_aug", False):
        ...
        post_transform = transforms.Compose([
            transforms.RandomHorizontalFlip(),   # optional, like AdaFace
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.5]*3, std=[0.5]*3),
        ])

        trainset = AugmentRecordDataset(
            root_dir=cfg.rec_root,              # e.g., 'faces_emore'
            transform=post_transform,            # runs AFTER AdaFace augmenter
    ...
    def _build_adaface_res50():
        ...
        checkpoint = torch.load("/home/ragulapa/surc/workspace/KD_FR/output/A_Teacher_updated/adaface_ir50_ms1mv2.pth", map_location=torch.device(local_rank))
    ...
    logging.info(f"teacher {cfg.teacher} loaded successfully!")
    logging.info(f"student {cfg.network} created successfully!")
    logging.info("train dataset: {}".format(cfg.dataset))
    logging.info(f"loss function: {cfg.loss}")
    logging.info(f"margin m: {cfg.m}")
    logging.info(f"scale s: {cfg.s}")
    logging.info("adaptive weighted alpha: {}".format(cfg.adaptive_alpha))
    logging.info("batch size per GPU: {}".format(cfg.batch_size))
    ...
    if getattr(cfg, "use_aroface_aug", False):
        ...
        module_partial_fc.train().cuda(local_rank)
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, label) in enumerate(train_loader):
            ...
            if cfg.loss == "AdaFace":
                thetas ,target_logit_mean, lma, cos_theta_tmp = header(F.normalize(features),F.normalize(features_t) , norms, label)
            else:
